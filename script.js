...
const MARKETS = [
  ...
  { id: 'jse', name: 'JSE - JOHANNESBURG', shortName: 'JSE', timezone: 'Africa/Johannesburg', openTime: '09:00', closeTime: '17:00', region: 'Europe' }, // Grouped under Europe for simpler UI
  ...
  { id: 'syd', name: 'ASX - SYDNEY', shortName: 'Australia/Sydney', timezone: 'Australia/Sydney', openTime: '10:00', closeTime: '16:00', region: 'Asia & Pacific' },
...
const getMarketStatus = (market, now) => {
  ...
  // Calculate WIB hours for timeline
...
const MarketTimeline = ({ status, now }) => {
  ...
  // Helper to check if a minute is within a range (handles overflow)
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      <div className="relative h-4 bg-slate-900/50 rounded-md overflow-hidden border border-slate-800/50 flex">
        {/* Draw segments for 1440 minutes */}
        {/* For efficiency, we'll draw 24 blocks but color them based on precise ranges */}
        {Array.from({ length: 24 }).map((_, h) => {
          ...
          // Simplified: if half of the hour is in a range, color it. 
          // For 100% precision we'd use absolute positioning.
          ...
          if (isTrading) {
            colorClass = inLunch ? 'bg-blue-500/50' : 'bg-orange-500/70';
          }
            ...
            <div key={h} className={`flex-grow h-full border-r border-slate-800/20 last:border-0 ${colorClass} transition-colors relative`}>
              {isCurrent && (
                <div 
                  ...
                  style={{ left: `${(now.getMinutes() / 60) * 100}%` }}
                ></div>
              )}
            </div>
          );
    ...
    <tr className="hover:bg-slate-800/20 transition-all group border-b border-slate-800/30 last:border-0">
      ...
      <td className="px-4 py-4">
        <div className="flex flex-col gap-1">
          <div className={`inline-flex items-center justify-center px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border w-20 ${
            m.isOpen ? 'bg-orange-500/20 border-orange-500/30 text-orange-400' : 
            m.isLunch ? 'bg-blue-500/20 border-blue-500/30 text-blue-400' : 
            'bg-slate-900 border-slate-800 text-slate-600'
          }`}>
            {m.statusText}
          </div>
          <span className={`text-[9px] font-mono text-center ${m.isOpen ? 'text-orange-500/70' : m.isLunch ? 'text-blue-500/70' : 'text-slate-600'}`}>
            {m.countdownText}
          </span>
        </div>
      </td>
      ...
      <td className="px-4 py-4">
        <div className="flex flex-col items-center">
          ...
          {m.lunchStart && (
            <div className="text-[8px] text-blue-500/60 font-mono mt-0.5">
      ...
      <td className="px-6 py-4 min-w-[200px]">
        <MarketTimeline status={m} now={now} />
      </td>
    </tr>
  );
};
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-col items-center md:items-end gap-1 bg-slate-950/50 px-6 py-2 rounded-2xl border border-slate-800/50 shadow-inner">
             ...
             <div className="flex items-center gap-2 text-[9px] text-slate-500 font-mono uppercase tracking-widest">
               <Clock size={10} /> WIB (UTC+7) Banyumas
             </div>
          </div>
        </div>
      </header>
      ...
      <main className="flex-grow p-4 md:p-8 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-8">
          <div className="flex-grow relative group">
            ...
            <input 
              type="text" 
              placeholder="Filter by Market, Region, or Symbol..." 
              className="w-full bg-slate-900/60 border border-slate-800 rounded-2xl pl-12 pr-12 py-4 text-sm focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/20 transition-all shadow-xl"
              value={searchTerm}
            ...
            {searchTerm && (
              <button 
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 p-1 hover:bg-slate-800 rounded-full text-slate-500 hover:text-white transition-colors"
              >
                <X size={16} />
              </button>
            )}
          ...
          {filteredMarkets.length === 0 ? (
            <div className="py-20 text-center flex flex-col items-center gap-4 bg-slate-900/20 rounded-3xl border border-dashed border-slate-800">
              ...
              <div className="space-y-1">
                <p className="text-slate-400 font-bold">No markets found</p>
                <p className="text-sm text-slate-600">Try searching for a different name or region.</p>
              </div>
            </div>
          ) : (
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-black text-white tracking-widest text-sm uppercase">HDWeb Monitor</span>
              </div>
              <p className="text-slate-500 text-xs max-w-sm leading-relaxed">
                Visualisasi waktu nyata sesi perdagangan bursa saham global dalam format 24 jam. 
                Data dikonversi ke Waktu Indonesia Barat (WIB) untuk memudahkan trader lokal.
              </p>
              <p className="text-slate-600 text-[10px] font-mono uppercase tracking-tighter">
                © {new Date().getFullYear()} HDWeb. Crafted in Banyumas, Indonesia.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-8 md:justify-end">
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2">
                  <a href="#" className="text-slate-500 hover:text-orange-500 text-xs flex items-center gap-2 transition-colors">
                    <Info size={12} /> Data Source Info
                  </a>
                  <a href="#" className="text-slate-500 hover:text-orange-500 text-xs flex items-center gap-2 transition-colors">
                    <ExternalLink size={12} /> Developer API
                  </a>
                </div>
              </div>
              <div className="space-y-4">
